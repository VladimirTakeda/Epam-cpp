...
SemWrapper::SemWrapper(std::string semName, int initialValue) : m_semName(std::move(semName)), m_semaphore(nullptr), m_created(false) {
...
SemMutexWrapper::SemMutexWrapper(std::unique_ptr<SemWrapper> &semWrapper) : m_Sem(semWrapper) {
    sem_wait(m_Sem->Get());
}
